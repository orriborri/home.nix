...
let
  ...
  rightModules = [ "custom/screenshot" "custom/kanshi" "custom/power-profile" "custom/audio" "network" "memory" "cpu" "custom/temperature" "battery" "sway/language" "tray" "clock#date" "clock#time" ];
  rightColors = [ "@brblue" "rgba(40, 40, 40, 0.8784313725)" "@power" "@sound" "@network" "@memory" "@cpu" "@temp" "@battery" "@layout" "@date" "@date" "@time" ];
  
  rightPowerline = if powerlineLib != null then powerlineLib.mkRightPowerline rightModules rightColors else {
    arrows = {};
  ...
  programs.waybar = {
    ...
    settings = {
      mainBar = {
        ...
        "custom/audio" = {
          ...
          on-click = "pactl list sinks short | cut -f2 | rofi -dmenu -p 'Output Device:' | xargs -I {} pactl set-default-sink {}";
          on-click-right = "pactl list sources short | grep -v monitor | cut -f2 | rofi -dmenu -p 'Input Device:' | xargs -I {} pactl set-default-source {}";
        ...
        "custom/screenshot" = {
          exec = "echo '📷'";
          format = "{}";
          tooltip-format = "Screenshot - Click for menu";
        ...
        "custom/power-profile" = {
          ...
          interval = 5;
          format = "{}";
          on-click = "/home/orre/.nix-profile/bin/powerprofilesctl list | grep -E '^\s*\*?\s*[a-z-]+:' | sed 's/^\s*\*\?\s*\([^:]*\):.*/\1/' | rofi -dmenu -p 'Power Profile:' | xargs -I {} /home/orre/.nix-profile/bin/powerprofilesctl set {}";
          tooltip-format = "Click to select power profile";
        };
    ...
    style = ''
      ...
      * {
      ...
      /* Workspaces with rounded corners */
      #workspaces {
      ...
      /* All modules with rounded corners */
      #clock,
      #cpu,
      #language,
      #memory,
      ...
      #battery.warning.discharging {
        background: linear-gradient(180deg, #fab387, #f9e2af);
        color: #1e1e2e;
      }
