...
let
  focusTerminal = pkgs.writeShellScriptBin "gnome-focus-terminal" ''
    # Find an existing alacritty window
    WIN=$(${pkgs.xdotool}/bin/xdotool search --class Alacritty | head -1)

    if [ -n "$WIN" ]; then
      CURRENT_WS=$(${pkgs.xdotool}/bin/xdotool get-desktop)
      ${pkgs.xdotool}/bin/xdotool set-desktop-for-window "$WIN" "$CURRENT_WS"
      ${pkgs.xdotool}/bin/xdotool windowactivate "$WIN"
    else
      alacritty -e zellij attach -c &
    fi
  '';
in
{
